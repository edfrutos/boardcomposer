...
class MainWindow(QMainWindow):
    ...
    def refresh_inspector_for_piece(self, piece_id: str):
        ...
        if placement is None:
            self.inspector.setText(
                ...
                f"{self._format_summary_area(piece.length_mm * piece.width_mm)}\n"
                f"{self._tr('inspector.thickness')}: "
    ...
    def _export_selected_solution(self):
        ...
        updated = dataclass_replace(
            ...
            export_include_panel_dimensions=options.include_panel_dimensions,
            export_include_plan_traceability=options.include_plan_traceability,
    ...
    def _sync_copy_inspector_action(self) -> None:
        ...
        tip = with_native_shortcuts(self._tr("tip.copy_inspector"))
        action.setStatusTip(
            tip
            if has_text
            else f"{tip} — {self._tr('status.nothing_to_copy_inspector')}"
        )
