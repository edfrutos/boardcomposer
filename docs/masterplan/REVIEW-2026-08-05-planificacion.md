# Revisión de planificación — 2026-08-05
...
listan bugs abiertos en la documentación de producto.
...
CLI, batch e HTTP de referencia. No es greenfield: el trabajo grande de
plataforma está entregado; el ciclo abierto es **QoL bajo demanda**.
...
4. **QoL `0.4.3`** — tomar slices de la cola candidata (abajo / DOC-004
   IDE-0019…0024) solo con demanda o al cerrar eval.
5. **No priorizar** IDE-0008 / LLM / DT-0006 C sin ADR o decisión vigente.

---

## 5. Cola candidata (ciclo `0.4.3` / post-eval)

Hay pendiente residual (eval + piloto), **no** cola de bugs documentados ni
épicas P0 abiertas. La cola grande (LLM, plugins, cloud C) está **bloqueada**.
Para no dejar el ciclo `0.4.3.dev0` sin candidatos concretos de producto, se
proponen ideas nuevas en `DOC-004` (IDE-0019…0024), ancladas a límites ya
escritos en MASTERPLAN / SCR-005 / ADR-010:

| ID | Título | Por qué ahora |
|----|--------|---------------|
| IDE-0019 | Intercambiar dos piezas | Límite Workspace documentado |
| IDE-0020 | Kerf / espesor de sierra | Anticipado en ADR-010 / SCR-005 |
| IDE-0021 | Restricción de veta | Metadatos de proyecto/pieza pendientes |
| IDE-0022 | Multipanel Skyline | Solo MaxRects cumple contrato hoy |
| IDE-0023 | Lista de corte / informe taller | Valor taller (usuarios N1–N2) |
| IDE-0024 | Metadatos de proyecto (cliente, ref., notas) | Evolución prevista SCR-005 |

Prioridad sugerida de ataque si se abre capacidad: **0024 → 0019 → 0020 →
0023 → 0021 → 0022** (UX/metadatos y swap antes de ampliar solvers).
...
- No se implementa código de producto en este pase: solo alinear docs y
  backlog.
- Bugs: sin evidencia en docs; Issues GitHub no consultables aquí.
- Próxima revisión automática: re-leer DOC-003/004/006 + CHANGELOG Unreleased
  y actualizar esta fecha o sustituir por `REVIEW-YYYY-MM-DD-…`.
