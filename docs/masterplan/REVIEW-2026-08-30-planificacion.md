# Revisión de planificación — 2026-08-30
...
revisión previa en `main` `REVIEW-2026-08-28-planificacion.md` (el snapshot
08-29 quedó en draft #582 sin merge; se incorpora aquí como histórico).  
**Issues GitHub:** `gh issue list --state open` → **vacío** (sin bugs abiertos
visibles en el repo). **PRs abiertos al corte:** #582 (docs planning 08-29,
draft) y #583 (tip honesty Quitar selección / Escape conserva tablero
enfocado).
...
| Tips honesty Guardar (última carpeta) + zoom Ctrl+0 + Explicar (sin IA red) + Seleccionar todas | 🟢 QoL `0.4.3.dev0` |
...
Desde la revisión 2026-08-28 en `main`, el ciclo `0.4.3.dev0` ya había cerrado
tips honesty Guardar (#576), zoom Ctrl+0 (#578/#579), Explicar candidata sin
IA en red (#580), Seleccionar todas (#581) y el snapshot 08-28 (#577). Entre
08-29 y este corte: **sin merges nuevos a `main`**; quedan abiertos el draft
de planificación #582 y el tip #583 (Quitar selección). Issues abiertos = 0.
...
- No se implementa código de producto en este pase: solo alinear docs y
  snapshot (incluye el histórico 08-29 que no llegó a `main`).
