...
**Origen:** snapshot post-`#628` (ciclo `0.4.4.dev0` ya en `main`).
Sustituye el draft cron `#629` (solapaba el bump de paquete).
...
**PRs abiertos al corte:** draft cron `#629` (supersedido por este snapshot).
...
incluido). Ciclo paquete `0.4.4.dev0` abierto (`#628`).

Desde la revisión 2026-09-14 (#624 mergeado), en `main` entró: merge `#623`
(IDE-0022), release `#627` (`0.4.3`), bump `#628` (`0.4.4.dev0`). Al corte
09-15: **Issues abiertos = 0**.
...
1. **Cola `0.4.4`** — atacar IDE-0025…0030 por prioridad sugerida
   (**0026 → 0027 → 0030 → 0025 → 0028 → 0029**).
...
Prioridad sugerida de ataque: **0026 → 0027 → 0030 → 0025 → 0028 → 0029**
(etiquetas y secuencia antes de inventario de retales; coste al final).
...
- No se implementa código de producto en este pase: solo alinear docs,
  snapshot y backlog con merges `#623`/`#627`/`#628` y cola vacía.
