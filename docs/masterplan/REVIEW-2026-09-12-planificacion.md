# Revisión de planificación — 2026-09-12
...
revisión previa `REVIEW-2026-09-11-planificacion.md` (PR #615 draft, plegado
aquí como histórico; `main` aún en corte 09-10 vía #613).  
**Issues GitHub:** `gh issue list --state open` → **vacío** (sin bugs abiertos
visibles en el repo). **PRs abiertos al corte:** 2 (`#614` tip honesty
Añadir tablero / cantidad = stock; `#615` planning 09-11 draft plegado).
...
| Tips honesty Añadir tablero (cantidad = stock paneles, no ids nuevos) | 🟡 PR `#614` abierto |
...
Desde la revisión 2026-09-10, en `main` no entró producto nuevo: solo planning
#613 (09-10). El snapshot 09-11 quedó en PR #615 draft (plegado aquí). Sin
merges de producto entre 09-10 y 09-12. Al corte 09-12: **Issues abiertos = 0**;
**PRs abiertos = 2** (`#614` tip; `#615` planning draft plegado).
...
4. **QoL / producto `0.4.3`** — atacar cola IDE-0019…0024 (DOC-004) cuando
   haya capacidad; la ola de tips/docs no vacía esa cola. Revisar/merge
   `#614` si tip Añadir tablero listo; cerrar/superseder `#615` tras merge
   de este snapshot.
...
- No se implementa código de producto en este pase: solo alinear docs y
  snapshot (incluye histórico 09-11 desde PR #615 draft).
