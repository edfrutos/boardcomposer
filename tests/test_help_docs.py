...
def test_solve_layout_tip_mentions_replaces_candidates():
    from studio.i18n import tr

    for key in ("tip.solve_layout", "tip.solve_layout_outdated"):
        es = tr(key, "es").casefold()
        en = tr(key, "en").casefold()
        assert "sustituye" in es and "candidatas" in es
...
def test_redo_tip_mentions_piece_board_or_placement_edit():
    from studio.i18n import tr

    es = tr("tip.redo", "es").casefold()
    en = tr("tip.redo", "en").casefold()
    assert "piezas" in es and "tableros" in es and "colocaciones" in es
    assert "piece" in en and "board" in en and "placement" in en
...
def test_zoom_status_tip_mentions_ignores_selection():
    from studio.i18n import tr
...
def test_next_solution_tip_mentions_without_applying():
    from studio.i18n import tr

    for key in ("tip.next_solution", "tip.next_solution_outdated"):
        es = tr(key, "es").casefold()
        en = tr(key, "en").casefold()
        assert "sin aplicarla" in es
        assert "without applying" in en
...
def test_timeline_filter_algorithm_tip_mentions_persists_across_sessions():
    from studio.i18n import tr

    es = tr("tip.timeline_filter_algorithm", "es").casefold()
    en = tr("tip.timeline_filter_algorithm", "en").casefold()
    assert "recuerda" in es and "sesiones" in es
    assert "remembered" in en and "sessions" in en


def test_timeline_filter_event_tip_mentions_persists_across_sessions():
    from studio.i18n import tr

    es = tr("tip.timeline_filter_event", "es").casefold()
    en = tr("tip.timeline_filter_event", "en").casefold()
    assert "recuerda" in es and "sesiones" in es
    assert "remembered" in en and "sessions" in en
...
def test_timeline_filter_markers_tip_mentions_persists_across_sessions():
    from studio.i18n import tr

    es = tr("tip.timeline_filter_markers", "es").casefold()
    en = tr("tip.timeline_filter_markers", "en").casefold()
    assert "recuerda" in es and "sesiones" in es
    assert "remembered" in en and "sessions" in en


def test_timeline_replay_mode_tip_mentions_persists_across_sessions():
    from studio.i18n import tr

    es = tr("tip.timeline_replay_mode", "es").casefold()
    en = tr("tip.timeline_replay_mode", "en").casefold()
    assert "recuerda" in es and "sesiones" in es
    assert "remembered" in en and "sessions" in en


def test_timeline_replay_speed_tip_mentions_persists_across_sessions():
    from studio.i18n import tr

    es = tr("tip.timeline_replay_speed", "es").casefold()
    en = tr("tip.timeline_replay_speed", "en").casefold()
    assert "recuerda" in es and "sesiones" in es
    assert "remembered" in en and "sessions" in en
...
def test_timeline_replay_back_tip_mentions_without_applying():
    from studio.i18n import tr

    es = tr("tip.timeline_replay_back", "es").casefold()
    en = tr("tip.timeline_replay_back", "en").casefold()
    assert "sin aplicarla" in es
    assert "without applying" in en


def test_timeline_replay_forward_tip_mentions_without_applying():
    from studio.i18n import tr

    es = tr("tip.timeline_replay_forward", "es").casefold()
    en = tr("tip.timeline_replay_forward", "en").casefold()
    assert "sin aplicarla" in es
    assert "without applying" in en
...
def test_timeline_clear_filters_tip_mentions_persists_across_sessions():
    ...
    es = tr("tip.timeline_clear_filters", "es").casefold()
    en = tr("tip.timeline_clear_filters", "en").casefold()
    assert "recuerda" in es and "sesiones" in es
    assert "remembered" in en and "sessions" in en
...
def test_save_tip_mentions_remembers_last_folder():
    from studio.i18n import tr

    es = tr("tip.save", "es").casefold()
    en = tr("tip.save", "en").casefold()
    assert "recuerda" in es and "carpeta" in es
    assert "remembers" in en and "folder" in en
...
def test_add_board_tip_mentions_creates_empty_project():
    from studio.i18n import tr
...
def test_add_board_tip_mentions_quantity_is_panel_stock():
    from studio.i18n import tr

    es = tr("tip.add_board", "es").casefold()
    en = tr("tip.add_board", "en").casefold()
    assert "cantidad" in es and "stock" in es
    assert "ids" in es
    assert "quantity" in en and "stock" in en
    assert "ids" in en
...
def test_add_piece_tip_mentions_places_on_first_board():
    from studio.i18n import tr
...
def test_toggle_toolbar_tip_mentions_persists_across_sessions():
    from studio.i18n import tr
...
def test_toggle_comparator_tip_mentions_persists_across_sessions():
    from studio.i18n import tr
